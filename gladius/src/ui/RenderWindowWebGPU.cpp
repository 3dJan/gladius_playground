...
namespace gladius::ui
{
    ...
    void RenderWindow::renderWindow()
    {
        ...
        if (!ImGui::IsAnyItemActive() || m_cameraDragButton >= 0)
        {
                                                 ...
                                                 ImVec2 const & contentMax)
    {
        ...
        bool cameraMotionApplied = false;
        if (m_cameraDragButton >= 0)
        {
            cameraMotionApplied = m_camera.mouseMotionHandler(mousePos.x, mousePos.y);
            if (cameraMotionApplied)
            {
                invalidateCameraView();
            }
        }

        bool const clickedLeft = ImGui::IsMouseClicked(ImGuiMouseButton_Left);
        bool const clickedRight = ImGui::IsMouseClicked(ImGuiMouseButton_Right);
        bool const clickedMiddle = ImGui::IsMouseClicked(ImGuiMouseButton_Middle);
        if (m_cameraDragButton >= 0)
        {
        ...
        else if (contentHovered)
        {
            ...
            {
                bool const clicked = (button == ImGuiMouseButton_Left && clickedLeft) ||
                                     (button == ImGuiMouseButton_Right && clickedRight) ||
                                     (button == ImGuiMouseButton_Middle && clickedMiddle);
                if (clicked)
                {
