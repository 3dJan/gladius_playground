...
namespace gladius::ui
{
    ...
    {
        // OpenCL path is implemented in SliceView.cpp (only compiled with OpenCL enabled).
        return false;
    }
    ...
    bool SliceView::renderImpl(ComputeCore * core, GLView & view)
    {
        ...
        // Document-based (WebGPU) contour generation. The generation is synchronous
        // (GPU dispatch + readback + CPU contour building), so it is throttled and
        // cached: contours are only regenerated when parameters changed or the
        // throttle interval elapsed (keeps slider dragging responsive).
        if (core == nullptr && m_document != nullptr)
        {
            bool const paramsChanged = !m_webGpuCacheValid;
            auto const now = std::chrono::steady_clock::now();
            auto const elapsed = now - m_lastWebGpuGeneration;
            constexpr auto throttleInterval = std::chrono::milliseconds(100);
            if ((paramsChanged || elapsed > throttleInterval) &&
                m_document->dispatchStructuralUpdateIfReady())
            {
                try
                {
                    ...
                    m_contours = m_document->generateContourWebGpu(m_sliceZ_mm, sliceParameter);
                    m_lastWebGpuGeneration = std::chrono::steady_clock::now();
        ...
        {
            // OpenCL path handled in SliceView.cpp; nothing to do here.
        }
        ...
        if (ImGui::Begin("Slice", &m_visible, ImGuiWindowFlags_MenuBar))
        {
            ...
            if (m_contours.has_value())
            {
                ...
                if (m_renderNormals || m_renderSourceVertices)
                {
        ...
        // Show a progress indicator while a WebGPU slice generation is pending
        if (core == nullptr && !m_webGpuCacheValid)
        {
