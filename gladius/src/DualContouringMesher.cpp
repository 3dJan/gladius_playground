...
namespace gladius::dual_contouring
{
    namespace
    {
        ...
        [[nodiscard]] size_t flattenIndex(size_t x,
                                          size_t y,
                                          size_t z,
                                          size_t width,
                                          size_t height)
                                                          ...
                                                          Eigen::Vector3f const & position)
        {
            ...
            for (int axis = 0; axis < 3; ++axis)
            {
                ...
                Eigen::Vector3f const forward =
                  clampToBounds(position + offset, minBounds, maxBounds);
                Eigen::Vector3f const backward =
                  clampToBounds(position - offset, minBounds, maxBounds);
        ...
        void emitTriangle(int idx0,
                          ...
                          DualContouringMesh & mesh,
                          DualContouringDiagnostics * diagnostics)
        {
            ...
            Eigen::Vector3f edge1 =
              mesh.vertices[static_cast<size_t>(v1)] - mesh.vertices[static_cast<size_t>(v0)];
            Eigen::Vector3f edge2 =
              mesh.vertices[static_cast<size_t>(v2)] - mesh.vertices[static_cast<size_t>(v0)];
            auto recomputeNormal = [&]() -> Eigen::Vector3f
            {
                edge1 = mesh.vertices[static_cast<size_t>(v1)] -
                        mesh.vertices[static_cast<size_t>(v0)];
                edge2 = mesh.vertices[static_cast<size_t>(v2)] -
                        mesh.vertices[static_cast<size_t>(v0)];
            ...
            Eigen::Vector3f expected = cellRefs[idx0]->normal + cellRefs[idx1]->normal +
                ...
                for (size_t j = 1U; j < height - 1U; ++j)
                {
                    for (size_t k = 1U; k < depth - 1U; ++k)
                    {
                        ...
                        std::array<std::tuple<int, int, int>, 4> const coords{
                          std::tuple<int, int, int>{static_cast<int>(i),
                                                     static_cast<int>(j - 1U),
                                                     static_cast<int>(k - 1U)},
                          std::tuple<int, int, int>{static_cast<int>(i),
                                                     static_cast<int>(j),
                                                     static_cast<int>(k - 1U)},
                          std::tuple<int, int, int>{static_cast<int>(i),
                        ...
                        for (size_t idx = 0U; idx < coords.size(); ++idx)
                        {
                            ...
                            if (cx < 0 || cy < 0 || cz < 0 || cx >= static_cast<int>(cellWidth) ||
                                cy >= static_cast<int>(cellHeight) ||
                                cz >= static_cast<int>(cellDepth))
                            {
                                skip = true;
                                break;
                            }
            ...
            for (size_t j = 0U; j < height - 1U; ++j)
            {
                for (size_t i = 1U; i < width - 1U; ++i)
                {
                    for (size_t k = 1U; k < depth - 1U; ++k)
                    {
                        ...
                        std::array<std::tuple<int, int, int>, 4> const coords{
                          std::tuple<int, int, int>{static_cast<int>(i - 1U),
                                                     static_cast<int>(j),
                                                     static_cast<int>(k - 1U)},
                          std::tuple<int, int, int>{static_cast<int>(i),
                                                     static_cast<int>(j),
    ...
    AxisAlignedBoundingBox DualContouringMesh::bounds() const
    {
    ...
    DualContouringMesh buildDualContouringMesh(OctreeBuilder const & builder,
                                               ...
                                               DualContouringDiagnostics * diagnostics)
    {
        ...
        if (diagnostics != nullptr)
        {
            ...
            diagnostics->signChangedEdgeCount = 0U;
